...
int main(void) {
  ...
  printf("TFT + Capacitive touch demo starting...\r\n");

  // Initialize TFT and capacitive touch
  TFT_Init();
  // CTP_Init();

  // Simple test pattern on TFT
  int m = 160;
  ...
  // printf("Touch the screen to draw!\r\n");
  ...
  while (1) {
    ...
    /* USER CODE BEGIN 3 */
    // ctp_touch_t touch;
    // if (CTP_GetTouch(&touch)) {
    //   // Draw a small 5x5 square at the touch point
    //   uint16_t x = touch.x;
    //   uint16_t y = touch.y;

    //   printf("Touch: x=%u y=%u\r\n", x, y);

    //   if (x >= 2 && y >= 2 && x < (TFT_WIDTH - 2) && y < (TFT_HEIGHT - 2)) {
    //     TFT_FillRect(x - 2, y - 2, 5, 5, COLOR_CYAN);
    //   } else {
    //     TFT_DrawPixel(x, y, COLOR_CYAN);
    //   }

    //   HAL_Delay(50); // basic debouncing
    // } else {
    //   // No touch; you could do something else or just idle
    //   HAL_Delay(10);
    // }
  }
  /* USER CODE END 3 */
